...
configFilePath = '/test/find-new-liquidity.json'
...
try:
    ...
    input("Please check that it syntactically correct with no missing/extra colons, quotation marks, curly brackets or omitted entries, or that data types are correct, and restart the program.")
    exit()
...
try:
    ...
    print("FATAL ERROR: Cannot connect to BSC node. This could be because your internet connection is unavailable, the BSC node has connectivity issues or a firewall is blocking the program's access.")
    input("Please fix the error and restart the program.")
    exit()
    ...
    try:
        ...
        if(jsonEventContents['args']['token0'] == liquidityPairAddress or jsonEventContents['args']['token1'] == liquidityPairAddress): #check if pair is WBNB, if not then ignore it
           ...
           # print("got tokenname and symbol")
            print (" [Token] New potential token detected: " + tokenName + " (" + tokenSymbol + "): " +  tokenAddress)
            requests.get('https://www.APIURL.com/get_new_token.php?name='+str(tokenName)+'&symbol='+str(tokenSymbol)+'&token='+str(tokenAddress))
        ...
        print("[ERROR 138] Please notify dev: ")
...
async def tokenLoop(event_filter, poll_interval):
    while True:
        try:
            ...
            print("[ERROR 162] Please notify dev: ")
...
try:
    ...
    print("[ERROR 184] Please notify dev: ") 
    listenForTokens()
